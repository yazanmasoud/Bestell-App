let meals = [
   {
    "category" : "Burger & Sandwiches",
    "name" : "Verggie mashroom black burger",
    "ingredients" : "Mixed green salad, Tomatoes, Edamame, Mushrooms",
    "price" : 16.90,
    "image" : "./assets/images/meals/burgers/veggieMashroomBurger.png"
   },
   {
    "category" : "Burger & Sandwiches",
    "name" : "All meat burger",
    "ingredients" : "Beef, Bacon, Dill pickles, smoked cheese, Ketchup, BBQ souse",
    "price" : 15.90,
    "image" : "./assets/images/meals/burgers/allMeatBurger.png"
   },
   {
    "category" : "Burger & Sandwiches",
    "name" : "Beef red burger",
    "ingredients" : "Beef, Cheese, Tomatoes, Lettuce, Onion",
    "price" : 14.90,
    "image" : "./assets/images/meals/burgers/beefRedBurger.png"
   },
   {
    "category" : "Burger & Sandwiches",
    "name" : "Big chicken burger",
    "ingredients" : "Chicken, Cheese, Tomatoes, Lettuce, Onion, Bell pepper, ",
    "price" : 14.90,
    "image" : "./assets/images/meals/burgers/bigChickenBurger.png"
   },

   /* pizza */
   {
    "category" : "Pizza",
    "name" : "pizza Margherita",
    "ingredients" : "Tomato Sauce, Mozzarella",
    "price" : 11.90,
    "image" : "./assets/images/meals/pizza/margharita.png"
   },
   {
    "category" : "Pizza",
    "name" : "Pizza Chorizo",
    "ingredients" : "Tomato slices, Mozzarella, Chorizo",
    "price" : 13.90,
    "image" : "./assets/images/meals/pizza/chorizo.png"
   },
   {
    "category" : "Pizza",
    "name" : "Funghi ",
    "ingredients" : "Red onion, Olives, Button Mushrooms, Mozzarella",
    "price" : 12.90,
    "image" : "./assets/images/meals/pizza/funghi.png"
   },
   {
    "category" : "Pizza",
    "name" : "Quattro Formaggi with Chicken",
    "ingredients" : "Chicken, Mozzarella, Gorgonzola, Fontina,  Parmigiano Reggiano",
    "price" : 15.90,
    "image" : "./assets/images/meals/pizza/quatroFormaggi.png"
   },
   /* salad */
   {
    "category" : "Salad",
    "name" : "Warm beef arugula salad",
    "ingredients" : "Beef, Arugula, Field salad, Greek feta, Cherry tomatoes, Sun-dried Tomatoes, Balsamic-vinegar dressing",
    "price" : 16.90,
    "image" : "./assets/images/meals/salad/arugulaSalad.png"
   },
   {
    "category" : "Salad",
    "name" : "Mini green Salad",
    "ingredients" : "Green salad, Cucumber, Carrots, Parsley, Radishes",
    "price" : 7.90,
    "image" : "./assets/images/meals/salad/miniGreenSalad.png"
   },
   {
    "category" : "Salad",
    "name" : "Green Salad with sea food",
    "ingredients" : "Mixed greens, Cherry tomatoes, Red onion, Mussels, Squid rings, Shrimp, Dijon mustard-lemon dressing with dill",
    "price" : 16.90,
    "image" : "./assets/images/meals/salad/seaFoodSalad.png"
   },
   {
    "category" : "Salad",
    "name" : "Vegan green salad with tofu",
    "ingredients" : " Green salad, Cherry tomatoes, Cucumber, Baby spinach, Edamame, Radishes, Bittercress, Tofu, Peanuts",
    "price" : 14.90,
    "image" : "./assets/images/meals/salad/tofuSalad.png"
   }
]

